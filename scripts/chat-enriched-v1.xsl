...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
	<!-- This stylesheet converts a BNC XML text into a series of numbered CHAT-CA files 
     separated by conversation number for re-transcription. -->
...
<xsl:variable name="docName" select="/bncDoc/@xml:id"/><xsl:result-document href="../cabnc_chat/{$docName}_{position()}.cha">@Begin
@Font:  CAfont:13:0
@Languages:     eng<xsl:apply-templates select="key('recTable',@n)"/><xsl:apply-templates/>
@End
</xsl:result-document>
	</xsl:template>
	...
	<xsl:variable name="id" select="@n"></xsl:variable>
@Participants:	<xsl:variable name="doc1" select="/"/>
		<xsl:for-each select="tokenize(@who, ' ')">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					...
					<xsl:value-of select="substring($shortName,1,3)"/>
				</xsl:otherwise>
			</xsl:choose>
			<xsl:text> </xsl:text>
			<xsl:value-of select="translate($doc1/bncDoc/teiHeader/profileDesc/particDesc/person[@xml:id=$speakerID2]/persName, ' ', '_')"/>
			<xsl:text> </xsl:text>
...
@Options:	CA<xsl:for-each select="tokenize(@who, ' ')">
			<xsl:variable name="speakerID2" select="."/>
...
@Location:	<xsl:value-of select="locale"/>
		...
		<xsl:if test="audioFile">
@Comment:	audio available at: http://bnc.phon.ox.ac.uk/data/<xsl:value-of select="audioFile"/>
		</xsl:if>
@Comment:	conversation #<xsl:number/> (Tape <xsl:value-of select="@n"/>)</xsl:template>
	...
	<xsl:template match="u">
		<xsl:variable name="speakerID" select="@who"/>
*<xsl:value-of select="substring(translate(/bncDoc/teiHeader/profileDesc/particDesc/person[@xml:id=$speakerID]/persName, 'abcdefghijklnmopqrstuvwxyz', 'ABCDEFGHIJKLNMOPQRSTUVWXYZ'),1,3)"/>
		<xsl:text>:	</xsl:text>
                <xsl:apply-templates/> 
	</xsl:template>
        <xsl:template match="s">
                ...
                <xsl:apply-templates select="mw|w|c"/>
		<xsl:text> •</xsl:text>
		<xsl:value-of select="translate(substring(@startTime,1,7), '.','')"/> 
		<xsl:text>_</xsl:text>
		<xsl:value-of select="translate(substring(@endTime,1,7), '.','')"/> 
                <xsl:text>•</xsl:text>
        </xsl:template>
